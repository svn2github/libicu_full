...
//  file:  crematch.cpp
//
//         Contains the implementation of class CSREMatcher,
...
static const UBool USE_COLLATION = TRUE;
...
//
//-----------------------------------------------------------------------------
CSREMatcher::CSREMatcher(const UCollator *coll, const RegexPattern *pat)  { 
...
CSREMatcher::CSREMatcher(const UCollator *coll, const UnicodeString &regexp, const UnicodeString &input,
...
CSREMatcher::CSREMatcher(const UCollator *coll, UText *regexp, UText *input,
                           uint32_t flags, UErrorCode &status) {
    fColl = coll;
    init(status);
    if (U_FAILURE(status)) {
...
CSREMatcher::CSREMatcher(const UCollator *coll, const UnicodeString &regexp, 
                           uint32_t flags, UErrorCode &status) {
    fColl = coll;
    init(status);
    if (U_FAILURE(status)) {
...
CSREMatcher::CSREMatcher(const UCollator *coll, UText *regexp, 
                           uint32_t flags, UErrorCode &status) {
    fColl = coll;
    init(status);
    if (U_FAILURE(status)) {
...
//            to run safely.
//
void CSREMatcher::init(UErrorCode &status) {
    fPattern           = NULL;
    fPatternOwned      = NULL;
    fFrameSize         = 0;
    fRegionStart       = 0;
...
//
//  init2()   Common initialization for use by CSREMatcher constructors, part 2.
//            This handles the common setup to be done after the Pattern is available.
//
void CSREMatcher::init2(UText *input, UErrorCode &status) {
    if (U_FAILURE(status)) {
        fDeferredStatus = status;
        return;
    }
...
//
//--------------------------------------------------------------------------------
CSREMatcher &CSREMatcher::appendReplacement(UnicodeString &dest,
...
//
CSREMatcher &CSREMatcher::appendReplacement(UText *dest,
                                              UText *replacement,
                                              UErrorCode &status) {
    if (U_FAILURE(status)) {
        return *this;
...
//
//--------------------------------------------------------------------------------
UnicodeString &CSREMatcher::appendTail(UnicodeString &dest) {
...
//
UText *CSREMatcher::appendTail(UText *dest) {
...
//
//--------------------------------------------------------------------------------
int32_t CSREMatcher::end(UErrorCode &err) const {
    return end(0, err);
}



int32_t CSREMatcher::end(int32_t group, UErrorCode &err) const {
    if (U_FAILURE(err)) {
        return -1;
    }
    if (fMatch == FALSE) {
...
UBool CSREMatcher::find(int32_t start, UErrorCode &status) {
    if (U_FAILURE(status)) {
        return FALSE;
    }
    if (U_FAILURE(fDeferredStatus)) {
...
//
//--------------------------------------------------------------------------------
UBool CSREMatcher::findUsingChunk() {
    // Start at the position of the last match end.  (Will be zero if the
    //   matcher has been reset.
    //
    ...
    case START_CHAR:
    {
        ...
        if (USE_COLLATION) {
            ...
            for (;;) {
                int32_t pos = startPos;
                UnicodeString pat = fPattern->pattern();
                ...
                if (matchLength >= 0) {
                    MatchChunkAt(pos, FALSE, fDeferredStatus);//TODO
                    if (U_FAILURE(fDeferredStatus)) {
                        ucol_closeElements(elems);
                        return FALSE;
                    }
...
//
//--------------------------------------------------------------------------------
UnicodeString CSREMatcher::group(UErrorCode &status) const {
    return group(0, status);
}

UText *CSREMatcher::group(UText *dest, MatcherDestIsUTextFlag /*flag*/, UErrorCode &status) const {
    return group(0, dest, status);
}



UnicodeString CSREMatcher::group(int32_t groupNum, UErrorCode &status) const {
...
UText *CSREMatcher::group(int32_t groupNum, UText *dest, UErrorCode &status) const {
    UBool bailOut = FALSE;
    if (U_FAILURE(status)) {
        bailOut = TRUE;
    }
...
int64_t CSREMatcher::appendGroup(int32_t groupNum, UText *dest, UErrorCode &status) const {
    int64_t destLen = utext_nativeLength(dest);
...
//
//--------------------------------------------------------------------------------
int32_t CSREMatcher::groupCount() const {
...
//
//--------------------------------------------------------------------------------
UBool CSREMatcher::hasAnchoringBounds() const {
...
//
//--------------------------------------------------------------------------------
UBool CSREMatcher::hasTransparentBounds() const {
...
//
//--------------------------------------------------------------------------------
UBool CSREMatcher::hitEnd() const {
    return fHitEnd;
}
...
//
//--------------------------------------------------------------------------------
const UnicodeString &CSREMatcher::input() const {
    if (!fInput) {
        UErrorCode status = U_ZERO_ERROR;
        int32_t len16;
        if (UTEXT_USES_U16(fInputText)) {
...
//
//--------------------------------------------------------------------------------
UText *CSREMatcher::inputText() const {
    return fInputText;
}
...
//
//--------------------------------------------------------------------------------
UText *CSREMatcher::getInput (UText *dest) const {
...
//
//--------------------------------------------------------------------------------
UBool CSREMatcher::lookingAt(UErrorCode &status) {
    if (U_FAILURE(status)) {
        return FALSE;
    }
    if (U_FAILURE(fDeferredStatus)) {
...
UBool CSREMatcher::lookingAt(int32_t start, UErrorCode &status) {
    if (U_FAILURE(status)) {
        return FALSE;
    }
    if (U_FAILURE(fDeferredStatus)) {
...
//
//--------------------------------------------------------------------------------
UBool CSREMatcher::matches(UErrorCode &status) {
    if (U_FAILURE(status)) {
        return FALSE;
    }
    if (U_FAILURE(fDeferredStatus)) {
...
UBool CSREMatcher::matches(int32_t start, UErrorCode &status) {
    if (U_FAILURE(status)) {
        return FALSE;
    }
    if (U_FAILURE(fDeferredStatus)) {
...
//
//--------------------------------------------------------------------------------
const RegexPattern &CSREMatcher::pattern() const {
    return *fPattern;
}
...
//
//--------------------------------------------------------------------------------
CSREMatcher &CSREMatcher::region(int32_t start, int32_t limit, UErrorCode &status) {
    if (U_FAILURE(status)) {
        return *this;
    }
...
//
//--------------------------------------------------------------------------------
int32_t CSREMatcher::regionEnd() const {
    if (UTEXT_USES_U16(fInputText)) {
...
//
//--------------------------------------------------------------------------------
int32_t CSREMatcher::regionStart() const {
    if (UTEXT_USES_U16(fInputText)) {
...
//
//--------------------------------------------------------------------------------
UnicodeString CSREMatcher::replaceAll(const UnicodeString &replacement, UErrorCode &status) {
    UText replacementText = UTEXT_INITIALIZER;
    UText resultText = UTEXT_INITIALIZER;
...
//
UText *CSREMatcher::replaceAll(UText *replacement, UText *dest, UErrorCode &status) {
    if (U_FAILURE(status)) {
...
//
//--------------------------------------------------------------------------------
UnicodeString CSREMatcher::replaceFirst(const UnicodeString &replacement, UErrorCode &status) {
    UText replacementText = UTEXT_INITIALIZER;
    UText resultText = UTEXT_INITIALIZER;
    UnicodeString resultString;
...
//
UText *CSREMatcher::replaceFirst(UText *replacement, UText *dest, UErrorCode &status) {
    if (U_FAILURE(status)) {
        return getInput(dest);
    }
    if (U_FAILURE(fDeferredStatus)) {
...
//
//--------------------------------------------------------------------------------
UBool CSREMatcher::requireEnd() const {
    return fRequireEnd;
}
...
void CSREMatcher::resetPreserveRegion() {
    fMatchStart     = 0;
    fMatchEnd       = 0;
    fLastMatchEnd   = -1;
    fAppendPosition = 0;
...
CSREMatcher &CSREMatcher::reset(const UnicodeString &input) {
...
CSREMatcher &CSREMatcher::reset(UText *input) {
    if (fInputText != input) {
...
/*CSREMatcher &CSREMatcher::reset(const UChar *) {
    fDeferredStatus = U_INTERNAL_PROGRAM_ERROR;
    return *this;
}*/


CSREMatcher &CSREMatcher::reset(int32_t position, UErrorCode &status) {
    if (U_FAILURE(status)) {
        return *this;
    }
...
//
//--------------------------------------------------------------------------------
void CSREMatcher::setTrace(UBool state) {
    fTraceDebug = state;
}
...
//
//---------------------------------------------------------------------
int32_t  CSREMatcher::split(const UnicodeString &input,
        UnicodeString    dest[],
...
//   split, UText mode
//
int32_t  CSREMatcher::split(UText *input,
        UText           *dest[],
        int32_t          destCapacity,
        UErrorCode      &status)
{
...
//
//--------------------------------------------------------------------------------
int32_t CSREMatcher::start(UErrorCode &status) const {
...
//
//--------------------------------------------------------------------------------
int32_t CSREMatcher::start(int32_t group, UErrorCode &status) const {
    if (U_FAILURE(status)) {
        return -1;
    }
    if (U_FAILURE(fDeferredStatus)) {
...
//
//--------------------------------------------------------------------------------
CSREMatcher &CSREMatcher::useAnchoringBounds(UBool b) {
    fAnchoringBounds = b;
...
//
//--------------------------------------------------------------------------------
CSREMatcher &CSREMatcher::useTransparentBounds(UBool b) {
...
//
//--------------------------------------------------------------------------------
void CSREMatcher::setTimeLimit(int32_t limit, UErrorCode &status) {
    if (U_FAILURE(status)) {
        return;
    }
    if (U_FAILURE(fDeferredStatus)) {
...
//
//--------------------------------------------------------------------------------
int32_t CSREMatcher::getTimeLimit() const {
    return fTimeLimit;
}
...
//
//--------------------------------------------------------------------------------
void CSREMatcher::setStackLimit(int32_t limit, UErrorCode &status) {
    if (U_FAILURE(status)) {
        return;
    }
    if (U_FAILURE(fDeferredStatus)) {
...
//
//--------------------------------------------------------------------------------
int32_t CSREMatcher::getStackLimit() const {
    return fStackLimit;
}
...
//
//--------------------------------------------------------------------------------
void CSREMatcher::setMatchCallback(URegexMatchCallback     *callback,
                                    const void              *context,
                                    UErrorCode              &status) {
    if (U_FAILURE(status)) {
        return;
...
//
//--------------------------------------------------------------------------------
void CSREMatcher::getMatchCallback(URegexMatchCallback   *&callback,
                                  const void              *&context,
                                  UErrorCode              &status) {
    if (U_FAILURE(status)) {
       return;
...
//--------------------------------------------------------------------------------
REStackFrame *CSREMatcher::resetStack() {
    // Discard any previous contents of the state save stack, and initialize a
...
//
//--------------------------------------------------------------------------------
UBool CSREMatcher::isWordBoundary(int64_t pos) {
...
UBool CSREMatcher::isChunkWordBoundary(int32_t pos) {
    UBool isBoundary = FALSE;
    UBool cIsWord    = FALSE;
...
//
//--------------------------------------------------------------------------------
UBool CSREMatcher::isUWordBoundary(int64_t pos) {
...
//
//--------------------------------------------------------------------------------
void CSREMatcher::IncrementTime(UErrorCode &status) {
    fTickCounter = TIMER_INITIAL_VALUE;
    fTime++;
    if (fCallbackFn != NULL) {
...
//
//--------------------------------------------------------------------------------
inline REStackFrame *CSREMatcher::StateSave(REStackFrame *fp, int64_t savePatIdx, UErrorCode &status) {
...
//
//--------------------------------------------------------------------------------
void CSREMatcher::MatchAt(int64_t startIdx, UBool toEnd, UErrorCode &status) {
...
//
//--------------------------------------------------------------------------------
void CSREMatcher::MatchChunkAt(int32_t startIdx, UBool toEnd, UErrorCode &status) {
    UBool       isMatch  = FALSE;      // True if the we have a match.
...
UOBJECT_DEFINE_RTTI_IMPLEMENTATION(CSREMatcher)
