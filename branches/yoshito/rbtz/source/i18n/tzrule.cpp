...
UOBJECT_DEFINE_RTTI_IMPLEMENTATION(TimeZoneRule)
...
AnnualTimeZoneRule::getStartInYear(int32_t year,
    ...
    if (fDateTimeRule->getTimeRuleType() != DateTimeRule::UNIVERSAL_TIME) {
...
TimeArrayTimeZoneRule::getUTC(UDate time, int32_t raw, int32_t dst) const {
    if (fTimeRuleType != DateTimeRule::UNIVERSAL_TIME) {
        time -= raw;
    }
