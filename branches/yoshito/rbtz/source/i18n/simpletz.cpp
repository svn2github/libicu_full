...
InitialTimeZoneRule*
SimpleTimeZone::getInitialRule(UErrorCode& status) /*const*/ {
    if (U_FAILURE(status)) {
        return NULL;
    }
    initTransitionRules(status);
...
TimeZoneRule*
SimpleTimeZone::getTransitionRule(int16_t index, UErrorCode& status) /*const*/ {
    if (U_FAILURE(status)) {
        return NULL;
    }
    initTransitionRules(status);
    ...
    if (index < 0 || index >= ((startMonth == 0) ? 0 : 2)) {
        status = U_INDEX_OUTOFBOUNDS_ERROR;
        return NULL;
    }
    return (index == 0) ? stdRule->clone() : dstRule->clone();
}
