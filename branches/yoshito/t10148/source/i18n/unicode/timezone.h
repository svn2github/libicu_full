...
public:
     ...
     * method sets the bogus flag to the result, but the operation itself remains successful
     ...
     *                  or umappable to a Windows time zone ID, then the bogus flag is set on return.
     ...
     *                  or umappable to a system time zone ID, then the bogus flag is set on return.
     * @param status    Recevies the status.
     * @return          A reference to the result (<code>id</code>).
     *
     * @draft ICU 52
     */
    static UnicodeString& U_EXPORT2 getIDByWindowsID(const UnicodeString& winid, const char* region,
