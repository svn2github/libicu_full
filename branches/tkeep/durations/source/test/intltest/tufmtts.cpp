...
void TimeUnitTest::testTimePeriodForAmounts() {
    ...
    }
    {
        TimePeriod period;
        if (!period.isBogus()) {
            errln("Expected time period to be bogus.");
        }
    }
    {
        UErrorCode status = U_ZERO_ERROR;
        ...
        if (!period.isBogus()) {
            errln("Expected period to be bogus.");
        }
    } 
    {
        UErrorCode status = U_ZERO_ERROR;
        TimePeriod period(NULL, 0, status);
        if (status != U_ILLEGAL_ARGUMENT_ERROR) {
...
void TimeUnitTest::testTimePeriodEquals() {
    ...
    _1h23s = create1h23s(status);
    TimePeriod _1h0m23s = create1h0m23s(status);
    TimePeriod _19m28s = create19m28s(status);
    TimePeriod _19m29s = create19m29s(status);
    ...
    // Different fields
    verifyNotEqual(_1h23s, _1h0m23s);

    // Same fields different values
    verifyNotEqual(_19m28s, _19m29s);
}
