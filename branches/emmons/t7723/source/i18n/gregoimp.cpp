...
CalendarData::CalendarData(const Locale& loc, const char *type, UErrorCode& status)
  : fBundle(NULL), fFallback(NULL) {
  initData(loc.getBaseName(), type, status);
}

void CalendarData::initData(const char *locale, const char *type, UErrorCode& status) {
  ures_initStackObject(&fFillin);
  ures_initStackObject(&fOtherFillin);
  ures_openFillIn(&fOtherFillin,U_CALENDAR_DATA, locale, &status);
  ures_getByKey(&fOtherFillin, U_CALENDAR_KEY, &fFillin, &status);
...
CalendarData::getByKey(const char *key, UErrorCode& status) {
    ...
    if(fBundle) {
        ures_getByKeyWithFallback(fBundle, key, &fFillin, &status);
#if defined (U_DEBUG_CALDATA)
    ...
    if(fFallback && (status == U_MISSING_RESOURCE_ERROR)) {
        ...
        ures_getByKeyWithFallback(fFallback, key, &fFillin, &status);
#if defined (U_DEBUG_CALDATA)
...
#endif
        ures_getByKeyWithFallback(fBundle, key, &fFillin, &status);
        ures_getByKeyWithFallback(&fFillin, U_FORMAT_KEY, &fOtherFillin, &status);
        ures_getByKeyWithFallback(&fOtherFillin, subKey, &fFillin, &status);
#if defined (U_DEBUG_CALDATA)
...
#endif
        ures_getByKeyWithFallback(fBundle, key, &fFillin, &status);
        ures_getByKeyWithFallback(&fFillin, contextKey, &fOtherFillin, &status);
        ures_getByKeyWithFallback(&fOtherFillin, subKey, &fFillin, &status);
#if defined (U_DEBUG_CALDATA)
