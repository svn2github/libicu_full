...
NumberingSystem::createInstance(const Locale & inLocale, UErrorCode& status) {
    ...
    if ( count > 0 ) { // @numbers keyword was specified in the locale
        ...
        ures_openFillIn(&resource,NULL, inLocale.getName(), &status);
...
NumberingSystem::createInstanceByName(const char *name, UErrorCode& status) {
     ...
     UResourceBundle nsCurrent,nsTop;
     ...
     ures_initStackObject(&nsTop);
     ures_initStackObject(&nsCurrent);
     ures_getByKey(numberingSystemsInfo,gNumberingSystems,&nsTop,&status);
     ures_getByKey(&nsTop,name,&nsTop,&status);
