...
SimpleDateFormat::processOverrideString(const Locale &locale, const UnicodeString &str, int8_t type, UErrorCode &status) {
    ...
    while (moreToProcess) {
        int32_t delimiterPosition = str.indexOf(ULOC_KEYWORD_ITEM_SEPARATOR_UCHAR,start);
        ...
        int32_t equalSignPosition = currentString.indexOf(ULOC_KEYWORD_ASSIGN_UCHAR,0);
