...
# Check for ELF
MESSAGE(STATUS "Checking for ELF binaries")
FILE(READ "${CMAKE_COMMAND}" EXE_HEADER LIMIT 4 HEX)
IF(EXE_HEADER STREQUAL 7f454c46)
    MESSAGE(STATUS "Checking for ELF binaries - Success")
    CHECK_INCLUDE_FILE(elf.h HAVE_ELF_HEADER)
    IF(HAVE_ELF_HEADER)
        SET(ELF32_TYPES Elf32_Ehdr Elf32_Off Elf32_Half Elf32_Shdr Elf32_Word Elf32_Sym)
        CHECK_SYMBOL_EXISTS(ELFCLASS64 elf.h HAVE_ELF64)
        IF(HAVE_ELF64)
            STRING(REPLACE 32 64 ELF64_TYPES "${ELF32_TYPES}")
        ENDIF()
        SET(CMAKE_EXTRA_INCLUDE_FILES elf.h)
        FOREACH(ELF_TYPE ${ELF32_TYPES} ${ELF64_TYPES})
            CHECK_TYPE_SIZE(${ELF_TYPE} HAVE_${ELF_TYPE})
            IF(NOT HAVE_${ELF_TYPE})
                SET(MISSING_ELF_TYPES TRUE)
                BREAK()
            ENDIF()
        ENDFOREACH()
        IF(NOT MISSING_ELF_TYPES)
            ADD_DEFINITIONS(-DU_ELF)
            SET(ELF_SYSTEM TRUE)
        ENDIF()
    ENDIF()
ELSE()
    MESSAGE(STATUS "Checking for ELF binaries - Failure")
ENDIF()

# Can genccode generate object files
IF(DEFAULT_BITNESS AND (WIN32 OR ELF_SYSTEM))
    SET(GENCCODE_OBJECT_GENERATION TRUE)
ENDIF()
