...
//#elif 0
//            UResourceBundle *confuser = NULL;
//            confuser = ures_getByIndex( patBundle, numberKeys-i, confuser, &err);
//            retPattern = ures_getString( confuser, &len, &err);
//            key = ures_getKey(confuser);
#endif

//fprintf(stderr, "Key4#%d %s: %s\n", i, DT_DateTimeAvailableFormatsTag, key);
    ...
    while((localeNameLen=uloc_getParent(parentLocale, parentLocale, 50, &err))>=0 ) {
        ...
        if (U_SUCCESS(err)) {
            ...
            for(i=0; i<numberKeys; ++i) {
                ...
                retPattern=ures_getNextString(patBundle, &len, &key, &err);
#endif            
//fprintf(stderr, "Key5 %s: %s\n", DT_DateTimeAvailableFormatsTag, key);
                UnicodeString format=UnicodeString(retPattern);
                UnicodeString retKey=UnicodeString(key, -1, US_INV);
