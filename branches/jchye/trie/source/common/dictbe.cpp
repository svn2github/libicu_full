...
CjkBreakEngine::CjkBreakEngine(const TrieWordDictionary *adoptDictionary, LanguageType type, UErrorCode &status)
    ...
    fKatakanaWordSet.applyPattern(UNICODE_STRING_SIMPLE("[:Katakana:]"), status);
...
// The codes is generated by ./katakana_heuristics_codegen.py (TODO: fix)
...
static inline uint32_t getKatakanaCost(int wordLength){
	//TODO: fill array with actual values!
...
static inline bool isKatakana(uint16_t value) {
  return (value >= 0x30A1u && value <= 0x30FEu && value != 0x30FBu) || 
		 (value >= 0xFF66u && value <= 0xFF9Du && value != 0xFF70u);
}
...
//        5 + (properties & AbstractCjkSegmenter::HAS_NON_KANJI_JAPANESE ? 5 : 0);
    //TODO: check range for non-kanji Jap chars!
