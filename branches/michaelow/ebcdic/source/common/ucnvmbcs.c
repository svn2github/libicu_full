...
ucnv_MBCSFromUnicodeWithOffsets(UConverterFromUnicodeArgs *pArgs,
    ...
    uint32_t asciiRoundtrips;
    uint32_t value;
    uint8_t siso_value[2], siso_value_length;
                ...
                case MBCS_OUTPUT_2_SISO:
                    ...
                    if(value<=0xff) {
                        if(value==0) {
                            ...
                            siso_value_length = getSISOBytes(SI, cnv->options, siso_value);
                            if (siso_value_length == 1) {
                                value|=siso_value[0]<<8;
                                length = 2;
                            } else if (siso_value_length == 2) {
                                value|=siso_value[1]<<8;
                                value|=siso_value[0]<<16;
                                length = 3;
                            }
                            prevLength=1;
                        }
                        ...
                        if(prevLength==2) {
                            ...
                            siso_value_length = getSISOBytes(SO, cnv->options, siso_value);
                            if (siso_value_length == 1) {
                                value|=siso_value[0]<<16;
                                length = 3;
                            } else if (siso_value_length == 2) {
                                value|=siso_value[1]<<16;
                                value|=siso_value[0]<<24;
                                length = 4;
                            }
                            prevLength=2;
                        }
...
unassigned:
    ...
    if( U_SUCCESS(*pErrorCode) &&
        ...
        siso_value_length = getSISOBytes(SI, cnv->options, siso_value);
        if(targetCapacity>0) {
            *target++=(uint8_t)siso_value[0];
            if (siso_value_length == 2) {
                *target++=(uint8_t)siso_value[1];
            }
            if(offsets!=NULL) {
            ...
            /* target is full */
            cnv->charErrorBuffer[0]=(uint8_t)siso_value[0];
            if (siso_value_length == 2) {
                cnv->charErrorBuffer[1]=(uint8_t)siso_value[1];
            }
            cnv->charErrorBufferLength=siso_value_length;
            *pErrorCode=U_BUFFER_OVERFLOW_ERROR;
        }
